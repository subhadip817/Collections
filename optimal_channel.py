...
st.title("🤖 AI-Driven Detection of Mode of Communication")
...
# Prediction Button
if st.button("🔮 Predict Optimal Channel"):
    X = pd.DataFrame([[
    ...
    pred_channel = channels[clf_channel.predict(X)[0]]
    pred_time = clf_time.predict(X)[0]

    st.info(f"📞 Optimal Follow-up Channel: **{pred_channel}**")
    st.info(f"🕒 Best Time to Follow-up: **{pred_time}:00 hrs**")